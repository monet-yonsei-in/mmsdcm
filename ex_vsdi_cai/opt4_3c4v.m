...
function minDCM = opt4_3c4v(SimName0, run_options)
...
cnt = 1;
boption(cnt).name = 'pE_Ainter_r';   boption(cnt).range = [-0.1 0.1 ]; cnt=cnt+1;
boption(cnt).name = 'pE_Aintra_r';   boption(cnt).range = [-0.1 0.1 ]; cnt=cnt+1;
boption(cnt).name = 'pE_Cr';   boption(cnt).range = [-0.1 0.1 ]; cnt=cnt+1;
boption(cnt).name = 'pE_Tr';   boption(cnt).range = [-0.1 0.1 ]; cnt=cnt+1;
    ...
    for jj = 1:size(M.pE.A, 2)
        if (M.pC.A(ii, jj) ~= 0)
            if  (ii <= 3 && jj <= 3 )
                ...
                %M.pE.A(ii, jj) = M.pE.A(ii, jj)  + bopt_test.pE_Aintra_r;
            ...
            elseif (ii > 12 && ii <= 14 ) && ( jj > 12 && jj <= 14 )
                ...
                M.pE.A(ii, jj) = M.pE.A(ii, jj) * ( 1 + bopt_test.pE_Ainter_r);
                % M.pE.A(ii, jj) = M.pE.A(ii, jj)  + bopt_test.pE_Ainter_r;
