...
function minDCM = opt2_3c4v(SimName0, run_options)
...
cnt = 1;
boption(cnt).name = 'pE_Ainter';   boption(cnt).range = [0.0 0.5 ]; cnt=cnt+1;
%boption(cnt).name = 'pE_C';   boption(cnt).range = [0.0 1.0 ]; cnt=cnt+1;
%boption(cnt).name = 'pE_T';   boption(cnt).range = [0.0 0.5 ]; cnt=cnt+1;
...
for ii = 1:size(M.pE.A, 1)
    for jj = 1:size(M.pE.A, 2)
        if (M.pC.A(ii, jj) ~= 0)
            if  (ii <= 3 && jj <= 3 )
                % M.pE.A(ii, jj) = bopt_test.pE_Aintra; %M.pE.A(ii, jj) * ( 1 + bopt_test.pE_Aintra_r);
...
%M.pE.C =   ( M.pC.C ~= 0 ) .* bopt_test.pE_C;
%M.pE.T =   ( M.pC.T ~= 0 ) .* bopt_test.pE_T;

%M.pE.A = (M.pC.A ~= 0) * bopt_test.pE_A;
%M.pE.A = (M.pC.A ~= 0) * bopt_test.pE_A;
%M.pE.C = (M.pC.C ~= 0) * bopt_test.pE_C;
%M.pE.T = (M.pC.T ~= 0) * bopt_test.pE_T;
...
yvsdi = GT2.Y.ymask.vsdi(1:100:end, 1:4);
yhat = [yvsdi(:); ycai(:)];
Y.y = yhat(:);
Y.dt = 0.1;
