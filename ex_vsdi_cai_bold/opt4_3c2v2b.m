function minDCM = opt4_3c2v2b(SimName0, run_options)
...
GTfile = './GT2.mat';
nodedef = 'node_fullmodel_opt3.csv';      % node
...
cnt = 1;
boption(cnt).name = 'pE_Ainter_r';   boption(cnt).range = [-0.3 0.3 ]; cnt=cnt+1;
...
ybold = GT2.Y.ymask.bold(:, 1:2);
yhat = [yvsdi(:); ycai(:); 0.1*ybold(:)];
Y.y = yhat(:);
Y.dt = 0.1;
