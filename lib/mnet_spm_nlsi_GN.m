function [Ep,Cp,Eh,F,L,dFdp,dFdpp] = mnet_spm_nlsi_GN(M,U,Y)
...
% added by jykang
if isfield(M.fH,'h_cai'),  M.fH.h_cai = spm_funcheck(M.fH.h_cai);  end
if isfield(M.fH,'h_bold'), M.fH.h_bold = spm_funcheck(M.fH.h_bold);  end

if isfield(M.hg,'vsdi'), M.hg.vsdi = spm_funcheck(M.hg.vsdi); end
if isfield(M.hg,'cai'),  M.hg.cai  = spm_funcheck(M.hg.cai);  end
if isfield(M.hg,'bold'), M.hg.bold = spm_funcheck(M.hg.bold); end
...
for k = 1:M.Nmax
    ...
    for m = 1:8
        ...
        dFdh  = dFdh  - ihC*d;
        dFdhh = dFdhh - ihC;
        Ch    = spm_inv(-dFdhh);   % curvature of hyperparameter
    ...
    % objective function: F(p) = log evidence - divergence, Friston et al. 2007 [Eq. 21]
    %----------------------------------------------------------------------
    ...
    if exist('Fsi', 'var')
        ...
        if isreal(spm_vec(y))
            if size(y, 2) ~=1
                %subplot(numPlt+2,1,1)
                xlabel(xLab)
                for n = 1:numPlt %M.nobs
                    subplot(numPlt+1,1,n)
                    if 1, %(n<=max(M.mC.Popu)) % observable signal-VSDI
                        plot(tidx,real(f(:,n)),'-','Color',cmap(n,:),'LineWidth',2);
                        set(gca,'XLim',[tidx(1) tidx(end)]);
                        yl=get(gca,'YLim'); d=abs(yl(2)-yl(1));
                        set(gca,'YLim',[yl(1)-0.1*d,yl(2)+0.1*d]);
                        hold on;
                        plot(tidx,real(fe(:,n)),':','Color',cmap(n,:)*2/3,'LineWidth',2);
                        hold off;
                    end                    
                end
            else                                    
                % TODO : seperate each signals
                subplot(2,1,1)
                plot(tidx,real(f(:)),'-','Color',cmap(1,:),'LineWidth',2);hold on;
                plot(tidx,real(fe(:)),':','Color',cmap(1,:)*2/3,'LineWidth',2);hold off;
            end
            ...
            subplot(4,2,1)
            plot(x,real(f),'LineWidth',2), hold on
            plot(x,real(f + e),':','LineWidth',2), hold off
            xlabel(xLab)
            ylabel('real')
            title(tstr,'FontSize',16)
            set( gca, 'FontSize',14,'LineWidth',2,'FontName','Arial Narrow');
            %grid on
        ...
        % subplot state x by jykang % mod. from KSJ spm_nlsi_GN_ksj
        %--------------------------------------------------------------
        prtstatex=0;
        if (prtstatex==1)
        %[~,statex] = jk_int_ode(Ep,M,U);  % filename shold be changed 
        %for n = 1:M.l
        for n = 1:max(M.mC.Popu)
            subplot(max(M.mC.Popu)+1,1,1+n)
            lgc      = M.mC.Popu == n;
            ntmp=find(lgc==1);
            for nx=1:sum(lgc)
                ntmp2=ntmp(nx);
                hplot = plot((1:1:size(statex(:,1),1))*U.dt,squeeze(statex(:,ntmp2)),'-','Color',cmap(nx,:),'LineWidth',2);
                if n,hold on;plot([x(1),x(end)],[M.x(1,1),M.x(1,1)],':k','LineWidth',2);end
                %if n <= M.nh,hplot.LineWidth = 2;hplot.LineStyle = ':';end
                if M.mC.ExIn(ntmp2)==-1,hplot.LineWidth = 2;hplot.LineStyle = ':';end
            end
            hold off;xlim([1,size(U.u,1)]*U.dt);ylim([-100,100])
            xlabel(xLab)
            title('cross-membrane potential','FontSize',16)
            set( gca, 'FontSize',14,'LineWidth',2,'FontName','Arial Narrow');    
            grid off; %on                        
        end
...
%         subplot(4,1,4)
%         for n = 1:M.l
%             hplot = plot((1:1:size(statex,1))*U.dt,squeeze(statex(:,M.l*2+n,1)),'-','Color',cmap(n,:),'LineWidth',2);
%             if n,hold on;plot([x(1),x(end)],[M.x(1,3),M.x(1,3)],':k','LineWidth',2);end
%             if n <= M.nh,hplot.LineWidth = 2;hplot.LineStyle = ':';end
%         end
%         hold off;xlim([1,size(U.u,1)]*U.dt);
%         xlabel(xLab)
        %title('concentration of calcium ions','FontSize',16)
%         title('spike-time','FontSize',16)
%         set( gca, 'FontSize',14,'LineWidth',2,'FontName','Arial Narrow');    
%         grid off; %grid on
%        end
        end
        ...
        % subplot parameters
        %--------------------------------------------------------------
        %subplot(max(M.mC.Popu)+2,1,max(M.mC.Popu)+2)
        %subplot(numPlt+2,1,numPlt+2)
        subplot(numPlt+1,1,numPlt+1)
        bar(full(V*p(ip)))
        xlim([0.5,length(V)+0.5])        
        xlabel('parameter')
        fieldindices = spm_fieldindices(pE,1:1:length(spm_vec(pE)));
        pos = get(gcf,'Position');
         set(gca,'XTick',1:1:length(V),'XTickLabel',fieldindices,'XTickLabelRotation',90); %'FontSize',pos(3)/numel(fieldindices));
%        set(gca,'XTick',1:1:length(V),'XTickLabel',fieldindices,'XTickLabelRotation',90,'FontSize',8);
        ...
        % for vsdi_cai
        ylim([-0.4,1.5])
        set( gca, 'FontName','Arial Narrow','FontSize',16);
        grid on
    ...
    criterion = [(dF < 1e-2) criterion(1:end - 1)]; %PHJ
    if all(criterion)
        if ~M.noprint
            fprintf(' convergence\n')
        end
